...
To keep recently active games and new purchases warm, selectors can be combined:

```powershell
./{{prefill_name}} prefill --played-within 365 --recently-purchased
```
...
| --played-within |     | days                  |             | Adds games last played within the specified number of days. Games that have never been played are excluded. |
| --recently-purchased |     |                       |             | Prefill will include all games purchased in the last 30 days. |
